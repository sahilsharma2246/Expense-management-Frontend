...
const Analytics = ({allTransection}) => {
  const categories = ["salary","tip","movie","food","bills","Tax","project","fee","medical"]
  

  //total transactions
  const totalTransection = allTransection.length
  const totalIncome = allTransection.filter(transaction=> transaction.type === 'income')
  const totalExpense = allTransection.filter(transaction=> transaction.type === 'expense')
